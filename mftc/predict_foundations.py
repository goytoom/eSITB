...
def predict(df_list, modes, thresholds):
    ...
    for mode, threshold in zip(modes, thresholds):
        ...
        for data_file in df_list:
            ...
            X_raw = list(df["full_text"]) # change to proper column name!
            ...
            # adjust naming convention to the files
            df_dropped.to_csv("../data/foundations/" + data_file.split(".")[0] + "_" + mode + ".csv", index=False)
...
df_list_raw = [] # Enter files to predict here!
df_list = [path + x for x in df_list_raw]

modes = ["moral", "binding", "full"]
thresholds = [0.5]

results = predict(df_list, modes = modes, thresholds = thresholds)
